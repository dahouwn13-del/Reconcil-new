...
public sealed class ExpediaForm : Form
{
    ...
    private List<ExpediaResultRecord> _results = new();
    ...
    private Control BuildUploadPanel()
    {
        ...
        table.Controls.Add(CreateFieldLabel("Expedia Excel"), 0, 1);
        table.Controls.Add(_expediaPath, 1, 1);
        var browseBooking = CreateBrowseButton();
        browseBooking.Click += (_, _) => BrowseExpedia();
    ...
    {
        using var dlg = new OpenFileDialog { Filter = "Expedia Excel (*.xls;*.xlsx)|*.xls;*.xlsx" };
        if (dlg.ShowDialog(this) == DialogResult.OK) _expediaPath.Text = dlg.FileName;
    }
    ...
    {
        if (!File.Exists(_expediaPath.Text) || !File.Exists(_operaPath.Text))
        {
        ...
        Logger.Info("Expedia reconciliation started.");
        Logger.Info($"Expedia file: {Path.GetFileName(_expediaPath.Text)}");
        ...
        try
        {
            ...
            {
                Logger.Info("Reading Expedia Excel file...");
                var bookings = ExpediaExcelReader.Read(_expediaPath.Text);
                Logger.Success($"Loaded {bookings.Count} Expedia records.");

                Logger.Info("Reading Opera Arrivals PDF...");
                var opera = ExpediaOperaPdfReader.Read(_operaPath.Text);
                Logger.Success($"Loaded {opera.Count} Opera records.");

                Logger.Info("Starting matching engine...");
                var results = ExpediaReconciliationEngine.Run(bookings, opera);
            ...
            _status.Text = $"Completed successfully — {_bookingCount} Expedia records and {_operaCount} Opera records processed.";
            _statusSummary.Text = $"Ready | Expedia: {_bookingCount:N0} | Opera: {_operaCount:N0} | Results: {_results.Count:N0} | Last run: {DateTime.Now:HH:mm}";
    ...
    private async Task ExportAsync()
    {
        ...
        using var save = new SaveFileDialog
        {
            ...
            FileName = $"Expedia_Reconciliation_{DateTime.Now:yyyy-MM-dd_HHmm}.xlsx"
        };
        ...
        try
        {
            await Task.Run(() => ExpediaExcelExporter.Save(save.FileName, _results, _bookingCount, _operaCount));
    ...
    private void AddEmptyCards() => UpdateCards(0, 0, Array.Empty<ExpediaResultRecord>());

    private void UpdateCards(int bookingCount, int operaCount, IReadOnlyList<ExpediaResultRecord> rows)
    {
        _cards.SuspendLayout();
        _cards.Controls.Clear();
        AddCard("All", "Expedia", bookingCount, Color.FromArgb(0, 119, 182));
        ...
        AddCard("Missing in Expedia", "Missing in Expedia", rows.Count(r => r.Result == "Missing in Expedia"), Color.FromArgb(153, 27, 27));
    ...
    {
        var selected = _activeFilter == filter || (_activeFilter == "All" && filter == "All" && label == "Expedia");
    ...
    {
        IEnumerable<ExpediaResultRecord> query = _results;
        ...
        {
            query = query.Where(r =>
                Contains(r.ReservationId, term) || Contains(r.ExpediaConfirmation, term) || Contains(r.ExpediaGuest, term) ||
    ...
    private void FormatGrid()
    {
        ...
        var friendly = new Dictionary<string, string>
        {
            ...
            [nameof(ExpediaResultRecord.ExpediaConfirmation)] = "Expedia Confirmation",
            [nameof(ExpediaResultRecord.ExpediaGuest)] = "Expedia Guest",
            [nameof(ExpediaResultRecord.ExpediaArrival)] = "Expedia Arrival",
            [nameof(ExpediaResultRecord.ExpediaDeparture)] = "Expedia Departure",
            [nameof(ExpediaResultRecord.ExpediaStatus)] = "Expedia Status",
            [nameof(ExpediaResultRecord.PaymentType)] = "Payment Type",
            [nameof(ExpediaResultRecord.BookingAmount)] = "Booking Amount",
            [nameof(ExpediaResultRecord.RoomDescription)] = "Room Description",
            [nameof(ExpediaResultRecord.OperaConf)] = "Opera Conf.",
            [nameof(ExpediaResultRecord.OperaGuest)] = "Opera Guest",
            [nameof(ExpediaResultRecord.OperaArrival)] = "Opera Arrival",
            [nameof(ExpediaResultRecord.OperaDeparture)] = "Opera Departure",
            [nameof(ExpediaResultRecord.OperaStatus)] = "Opera Status",
            [nameof(ExpediaResultRecord.OperaRoom)] = "Opera Room",
            [nameof(ExpediaResultRecord.MatchScore)] = "Score",
            [nameof(ExpediaResultRecord.MatchMethod)] = "Match Method",
            [nameof(ExpediaResultRecord.Result)] = "Result",
            [nameof(ExpediaResultRecord.Reason)] = "Reason",
            [nameof(ExpediaResultRecord.ActionRequired)] = "Action Required",
            [nameof(ExpediaResultRecord.NameAnalysis)] = "Smart Name Analysis"
        };
