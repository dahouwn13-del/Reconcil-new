...
public sealed class HomeForm : Form
{
    ...
    public HomeForm()
    {
        ...
        {
            Dock = DockStyle.Fill,
            Padding = new Padding(70, 35, 70, 35),
            ColumnCount = 1,
            RowCount = 5,
            BackColor = BackColor
        };
        ...
        var accountBar = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 3, RowCount = 1, BackColor = Color.Transparent };
        accountBar.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        accountBar.ColumnStyles.Add(new ColumnStyle(SizeType.AutoSize));
        accountBar.ColumnStyles.Add(new ColumnStyle(SizeType.AutoSize));
        accountBar.Controls.Add(new Label
        {
            Text = $"Signed in as {CurrentSession.User?.FullName} ({CurrentSession.User?.Role})",
            Dock = DockStyle.Fill,
            TextAlign = ContentAlignment.MiddleLeft,
            ForeColor = Color.FromArgb(71, 85, 105)
        }, 0, 0);

        if (CurrentSession.IsAdministrator)
        {
            var users = MakeTopButton("USER MANAGEMENT");
            users.Click += (_, _) => { using var form = new UserManagementForm(); form.ShowDialog(this); };
            accountBar.Controls.Add(users, 1, 0);
        }

        var logout = MakeTopButton("LOG OUT");
        logout.Click += (_, _) =>
        {
            AppServices.Users.Audit(CurrentSession.Username, "Logout", "User logged out");
            LogoutRequested = true;
            Close();
        };
        accountBar.Controls.Add(logout, 2, 0);
        root.Controls.Add(accountBar, 0, 0);
        ...
        root.Controls.Add(tiles, 0, 3);

        root.Controls.Add(new Label
        {
            Text = $"Logged in: {CurrentSession.Username}   |   {CurrentSession.User?.Role}   |   Version 6.0",
            Dock = DockStyle.Fill,
            TextAlign = ContentAlignment.MiddleCenter,
